# Quick Start: Using the HDF5 MCP with Gemini

This guide provides the fastest path to interacting with HDF5 using the `wrp` client and Google's Gemini LLM.
...
2. Install the HDF5 MCP either:
   ...
   Or individually:
   ```bash
   # Install just the HDF5 MCP
   uv pip install "git+https://github.com/iowarp/scientific-mcps.git@main#subdirectory=HDF5"
   ```
...
3.  Ensure `HDF5` is listed under the `MCP` section.
...
The client will start and connect to the HDF5 server.
